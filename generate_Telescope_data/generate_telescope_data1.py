...
#     output_file_name.append('data'+str(i)+'.pkl')
...
def matchSNR(signal, noise, psd): # signal's length is smaller than noise
    ...
    snr = np.abs(matched_filter(template=TimeSeries(template_use,delta_t=1/freq), data=TimeSeries(signal_plus_noise,delta_t=1/freq),psd=psd,low_frequency_cutoff=20,high_frequency_cutoff=4096))
...
def generate_antena_response(mass1_range, mass2_range, spin1z_range, spin2z_range,noiseE1,noiseE2,noiseE3,psd):
    ...
    for ii in range(10):
        ...
        #SpinTaylorT4,SEOBNRv4,TaylorF2,IMRPhenomD_NRTidal
        # start_time = time.time()
        hp, hc = get_td_waveform(approximant='IMRPhenomD_NRTidal',
                                 mass1=mass1,
                                 mass2=mass2,
                                 spin1z=spin1z,
                                 spin2z=spin2z,
                                 ...
                                 coa_phase=coa_phase,
                                 distance=100)
        signal_E1_template  = det_E1.project_wave(hp, hc, right_ascension, declination, polarization)
        signal_E2_template  = det_E2.project_wave(hp, hc, right_ascension, declination, polarization)
        signal_E3_template  = det_E3.project_wave(hp, hc, right_ascension, declination, polarization)

        # print("time:", time.time()-start_time)
        # plt.plot(signal_E1_template.get_sample_times(),signal_E1_template)
        # plt.show()

        # signal_E1_template.append_zeros(2048)
        # signal_E2_template.append_zeros(2048)
        # signal_E3_template.append_zeros(2048)
        length_E1 = signal_E1_template.shape[0]
        length_E2 = signal_E2_template.shape[0]
        length_E3 = signal_E3_template.shape[0]


        if length_E1 > freq * 100:
            signal_E1 = signal_E1_template[length_E1 - freq * 100:]
        else:
            signal_E1 = signal_E1_template
        if length_E2 > freq * 100:
            signal_E2 = signal_E2_template[length_E2 - freq * 100:]
        else:
            signal_E2 = signal_E2_template
        if length_E3 > freq * 100:
            signal_E3 = signal_E3_template[length_E3 - freq * 100:]
        else:
            signal_E3 = signal_E3_template

        del signal_E1_template, signal_E2_template, signal_E3_template
        gc.collect()
        ...
        snr_E3_list.append(matchSNR(signal_E2, noiseE3, psd))
...
class data_sample():
    def __init__(self, noiseE1, noiseE2, noiseE3, psd, mass1_list, mass2_list, spin1z_list, spin2z_list, right_ascension_list,
    ...
    def help(self):
        print('noiseE1, noiseE2, noiseE3 are 256 s length noise of E1, E2 and E3')
    ...
    for jj in range(44,50):
        ...
        for index in range(10):
            ...
            mass1_list, mass2_list, spin1z_list, spin2z_list, right_ascension_list, declination_list, signal_E1_list, signal_E2_list, signal_E3_list, snr_E1_list, snr_E2_list, snr_E3_list= (
                generate_antena_response((1,2), (1,2), (0,0.998), (0,0.998), noiseE1, noiseE2, noiseE3, psd))
            data.append(data_sample(np.array(noiseE1),np.array(noiseE2),np.array(noiseE3), np.array(psd), mass1_list,mass2_list,spin1z_list,spin2z_list,right_ascension_list,
                                    declination_list,signal_E1_list,signal_E2_list,signal_E3_list,snr_E1_list,snr_E2_list,snr_E3_list))
            print(str(jj*100+index*10+10), 'samples complete!')


        with open('BNS_100s_8192hz/'+'data'+str(jj+501)+'.pkl', 'wb') as f:
            pickle.dump(data, f)
