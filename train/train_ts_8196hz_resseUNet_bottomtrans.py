...
from load_data_yxy import get_train_batch_iter, data_sample
...
def normal_train():
    # model = seunet()
    model = keras.models.load_model('../model/train_peak_8192_ts_bottomtrans_reseunet_snr20-40_5s_0.0013814_10+30.h5', \
                                        custom_objects={"EncoderModel_6": EncoderModel_6})
    # model = keras.models.load_model('train_4096_ts2_transnotadd_reseunet_snr20-40_10s_0.0012987.h5', \
    #                  custom_objects={"pad_Layer":pad_Layer,"depad_Layer":depad_Layer,\
    #                                  "EncoderModel_6": EncoderModel_6})
    ...
    batch_size = 32
    sample_length = 5
    # snr_list=[40,30,25,20,25,30]
    snr_list=[20,15,12,10,12,15]
    peak_range=(0.6,0.9)
    sample_distance=[1,29]
    freq=8192
    snr_change_time=2000
    
    train_iter = get_train_batch_iter(batch_size=batch_size,sample_length=sample_length,snr_list=snr_list,\
                                           peak_range=peak_range,freq=freq, snr_change_time=snr_change_time,\
                                           datapath=train_datapath)
    test_iter = get_train_batch_iter(batch_size=batch_size,sample_length=sample_length,snr_list=snr_list,\
                                           peak_range=peak_range,freq=freq, snr_change_time=snr_change_time,\
                                           datapath=valid_datapath)
    check_point = ModelCheckpoint('../8192_trans/train_peak_8192_ts_bottomtrans_reseunet_snr20-40_5s_{val_loss:.7f}_{epoch:02d}.h5')
